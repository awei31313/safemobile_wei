...
import com.example.mobilesafe1.engine.VirusDao;
...
public class AnitVirusActivity extends Activity {
	...
	private ImageView iv_activity_anit_virus_scanning;
	private TextView tv_activity_anit_virus_name;
	private ProgressBar pb_activity_anit_virus_bar;
	private LinearLayout ll_activity_anit_virus_add_text;
	private int index = 0;
	...
	private Handler mHandler = new Handler(){
		public void handleMessage(android.os.Message msg) {
			...
			case SCANING:
				...
				tv_activity_anit_virus_name.setText(info.name);
				...
				ll_activity_anit_virus_add_text.addView(textView, 0);
				break;
			case SCAN_FINISH:
				tv_activity_anit_virus_name.setText("扫描完成");
				//停止真正执行的旋转动画
				iv_activity_anit_virus_scanning.clearAnimation();
				//告知用户卸载包含了病毒的应用
				unInstallVirus();
				break;
			}
	...
	private void checkVirus() {
		new Thread(){
			public void run() {
				...
				//设置进度条的最大值
				pb_activity_anit_virus_bar.setMax(packageInfoList.size());
				...
				for (PackageInfo packageInfo : packageInfoList) {
					...
					//7.在扫描的过程中,需要更新进度条
					index++;
					pb_activity_anit_virus_bar.setProgress(index);
...
//		rotateAnimation.setRepeatMode(RotateAnimation.INFINITE);
		...
		//一直执行动画
		iv_activity_anit_virus_scanning.startAnimation(rotateAnimation);
	}

	private void initUI() {
		iv_activity_anit_virus_scanning = (ImageView) findViewById(R.id.iv_activity_anit_virus_scanning);
		tv_activity_anit_virus_name = (TextView) findViewById(R.id.tv_activity_anit_virus_name);
		pb_activity_anit_virus_bar = (ProgressBar) findViewById(R.id.pb_activity_anit_virus_bar);
		ll_activity_anit_virus_add_text = (LinearLayout) findViewById(R.id.ll_activity_anit_virus_add_text);
	}
}
