...
public class CacheClearActivity extends Activity {
	...
	private Button bt_activity_cache_clear_clear;
	private ProgressBar pb_activity_cache_clear_bar;
	private TextView tv_activity_cache_clear_name;
	private LinearLayout ll_activity_cache_clear_add_text;
	...
	private Handler mHandler = new Handler(){
		public void handleMessage(Message msg) {
			...
			case UPDATE_CACHE_APP:
				//8.在线性布局中添加有缓存应用条目
				View view = View.inflate(getApplicationContext(), R.layout.item_activity_cacheclear_linearlayout, null);
				
				ImageView iv_icon = (ImageView) view.findViewById(R.id.iv_item_activity_cacheclear_linearlayout_icon);
				TextView tv_item_name = (TextView) view.findViewById(R.id.tv_item_activity_cacheclear_linearlayout_name);
				TextView tv_memory_info = (TextView)view.findViewById(R.id.tv_item_activity_cacheclear_linearlayout_memory_info);
				ImageView iv_delete = (ImageView) view.findViewById(R.id.iv_item_activity_cacheclear_linearlayout_delete);
				...
				ll_activity_cache_clear_add_text.addView(view, 0);
			...
			case CHECK_CACHE_APP:
				tv_activity_cache_clear_name.setText((String)msg.obj);
				break;
			case CHECK_FINISH:
				tv_activity_cache_clear_name.setText("扫描完成");
				break;
			case CLEAR_CACHE:
				//从线性布局中移除所有的条目
				ll_activity_cache_clear_add_text.removeAllViews();
				break;
			}
		};
	};
	...
	private void initData() {
		new Thread(){
			public void run() {
				...
				pb_activity_cache_clear_bar.setMax(installedPackages.size());
				//4.遍历每一个应用,获取有缓存的应用信息(应用名称,图标,缓存大小,包名)
				for (PackageInfo packageInfo : installedPackages) {
						
						synchronized(bt_activity_cache_clear_clear){
						...
						try {
							bt_activity_cache_clear_clear.wait();
...
//						}
						mIndex++;
						pb_activity_cache_clear_bar.setProgress(mIndex);
...
//					Log.e("重要", ""+cacheSize);
					...
					bt_activity_cache_clear_clear.notifyAll();
				}
			}
		};
		//1.获取指定类的字节码文件
	...
	private void initUI() {
		bt_activity_cache_clear_clear = (Button) findViewById(R.id.bt_activity_cache_clear_clear);
		pb_activity_cache_clear_bar = (ProgressBar) findViewById(R.id.pb_activity_cache_clear_bar);
		tv_activity_cache_clear_name = (TextView) findViewById(R.id.tv_activity_cache_clear_name);
		ll_activity_cache_clear_add_text = (LinearLayout) findViewById(R.id.ll_activity_cache_clear_add_text);
		
		bt_activity_cache_clear_clear.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				//1.获取指定类的字节码文件
				try {
